...
┃ ┗━━━━━━━━━━━━━━┈⊷
┃
┃ ┏━━━◈ *HACK COMMANDS* (Only For VIP & Special Users)━━━┈⊷
┃ ┃ ➥ *${config.prefix}getdp* - Get DP of someone
┃ ┗━━━━━━━━━━━━━━┈⊷
┃
┃ ┏━━━◈ *FINDER COMMANDS*━━━┈⊷
...
case 'lv': {
    ...
    let name1 = text[0].trim().toLowerCase();
    let name2 = text[1].trim().toLowerCase();

    // 🔢 සිරා ලොජික් එක - නම් දෙකේ අකුරු වල අගයන් එකතු කරලා ප්‍රතිශතයක් හදනවා
    let combinedString = [name1, name2].sort().join(""); // නම් දෙක මාරු කරලා ගැහුවත් එකම අගය එන්න sort කරනවා
    let sum = 0;
    for (let i = 0; i < combinedString.length; i++) {
        sum += combinedString.charCodeAt(i);
    }
    let percentage = (sum % 101); // හැමදාම 0-100 අතර එකම අගයක් එනවා

    // 🎭 ප්‍රතිශතය අනුව තත්ත්වය
    let status = "";
    if (percentage >= 90) status = "❤️ සැබෑ ආදරයක්! කිසිදා වෙන් කරන්න බෑ.";
    else if (percentage >= 70) status = "💖 ලස්සන ආදරයක්. ටිකක් උත්සාහ කළොත් සාර්ථකයි!";
    else if (percentage >= 50) status = "💛 හොඳ යාළුවෝ වගේ ඉන්න පුළුවන්.";
    else if (percentage >= 30) status = "💔 ගැලපීම ටිකක් අඩුයි වගේ...";
    else status = "💀 අනේ මන්දා ඉතින්... වැඩේ අමාරුයි!";

    let loveMsg = `*❤️ ALPHA-KING LOVE TESTER ❤️*\n\n` +
                 `*පළමු නම:* ${text[0].trim()}\n` +
                 `*දෙවන නම:* ${text[1].trim()}\n\n` +
                 `*ගැලපීම:* ${percentage}%\n` +
                 `*තත්ත්වය:* ${status}\n\n` +
                 `_සටහන: මෙය විනෝදය සඳහා පමණි!_`;

    await sock.sendMessage(remoteJid, { text: loveMsg }, { quoted: msg });
}
break;
